...
'''join refers to joining that specific thread with the main thread. If we're not joining with the main thread, then the time it shows is 
the actual execution time of the main thread. But if we join, then it shows the actual time it took for all the threads to finish execution 
...
# As you can see, the threading module provides a simple and efficient way to implement multithreading in Python. Whether you need to 
# create a new thread, run a function across multiple input values, or synchronize access to shared resources, the threading module has you covered.